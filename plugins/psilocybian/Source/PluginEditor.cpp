...
SpecraumAudioProcessorEditor::SpecraumAudioProcessorEditor (SpecraumAudioProcessor& p)
...
SpecraumAudioProcessorEditor::~SpecraumAudioProcessorEditor()
{
   #if JUCE_WINDOWS
...
void SpecraumAudioProcessorEditor::paint (juce::Graphics& g)
{
    g.fillAll (juce::Colour::fromRGB (9, 12, 19));
}

void SpecraumAudioProcessorEditor::resized()
{
    webView->setBounds (getLocalBounds());
}

void SpecraumAudioProcessorEditor::timerCallback()
{
    if (webView == nullptr)
        return;
...
juce::WebBrowserComponent::Options SpecraumAudioProcessorEditor::createWebOptions (SpecraumAudioProcessorEditor& editor)
{
    auto options = juce::WebBrowserComponent::Options{}
        ...
        .withNativeFunction ("setReferenceSpectrum",
            [&editor] (const juce::Array<juce::var>& args, juce::WebBrowserComponent::NativeFunctionCompletion done)
            {
                std::array<float, SpecraumAudioProcessor::spectrumBins> bins {};
                bool hasData = false;
                ...
                if (args.size() > 0)
                {
                    ...
                    {
                        const int num = juce::jmin (values->size(), SpecraumAudioProcessor::spectrumBins);
                        for (int i = 0; i < num; ++i)
                        {
...
std::optional<juce::WebBrowserComponent::Resource> SpecraumAudioProcessorEditor::getResource (const juce::String& url)
{
    ...
    if (isIndexPath)
    {
        return makeResource (specraum_BinaryData::index_html,
                             specraum_BinaryData::index_htmlSize,
                             "text/html");
    }
...
void SpecraumAudioProcessorEditor::toggleFullscreen()
{
   #if JUCE_WINDOWS
    ...
    fullscreenTarget = target;
    const bool wantFullscreen = ! fullscreen;
    const bool isStandalone = (juce::PluginHostType::getPluginLoadedAs()
    ...
    bool toggledByPeer = false;
    if (isStandalone)
    {
        if (auto* peer = fullscreenTarget->getPeer())
        {
            peer->setFullScreen (wantFullscreen);
            toggledByPeer = (peer->isFullScreen() == wantFullscreen);
        }
    }
    ...
    if (! toggledByPeer)
    {
        if (wantFullscreen)
        {
            ...
            auto userArea = display != nullptr ? display->userArea : juce::Rectangle<int> (0, 0, 1600, 1000);
            userArea = userArea.reduced (18);

            const int desiredWidth = juce::jmax (980, userArea.getWidth());
            const int desiredHeight = juce::jmax (620, userArea.getHeight());

            setSize (desiredWidth, desiredHeight);
        }
        else if (! windowedBounds.isEmpty())
        {
            setSize (windowedBounds.getWidth(), windowedBounds.getHeight());
        }
    }
